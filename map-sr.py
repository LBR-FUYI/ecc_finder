...
def main():
    ...
    map_options.add_argument("--large-genome", metavar="STR", type=str, default="", help="reference genome size larger than 4Gb [yes]")

    val_options = parser.add_argument_group("validation options")
    val_options.add_argument("-l", metavar="INT", type=int, default=200, help="minimum length of a peak [200]")
    val_options.add_argument("-d", metavar="INT", type=int, default=1000, help="maximum distance between signif. sites [1000]")
    val_options.add_argument("-p", metavar="FLT", type=float, default=0.05, help="maximum p-value [0.05]")
    val_options.add_argument("--min-read", metavar="INT", type=int, default=5, help="filter locus by unique mapped read number [5]")
    ...
    if not args.idx or not args.query1 or not args.query2 or not args.r :
        parser.print_help()
    ...
    d=pd.read_csv(output_path +file_prefix+".csv", sep='\t',header=None,names=['chr','rstart','rend','num_s','num_d','len'])
